...
Функция ВерсияАвтор()
	...
	МассивСтрок.Добавить("2.7 final");
	МассивСтрок.Добавить(НСтр("ru=' от '"));
	МассивСтрок.Добавить("13.11.2019");
...
Процедура УстановитьЗаголовокИнформацияОРелизе(Форма)
	...
	Если Форма.РелизыОснования.Количество() > 1 Тогда
...
Процедура ПутьКФайлуCFUНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ДопПараметры.Вставить("ИмяРеквизита", "ПутьКФайлуCFU");
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ДиалогВыбораФайлаЗавершение", ЭтотОбъект, ДопПараметры);
	
	ДиалогВыбора = Новый ДиалогВыбораФайла(РежимДиалогаВыбораФайла.Открытие);
	ДиалогВыбора.Фильтр			= НСтр("ru='Файлы обновлений конфигураций'") + " (*.cfu)|*.cfu";
	ДиалогВыбора.ПолноеИмяФайла	= Объект.ПутьКФайлуCFU;
	ДиалогВыбора.Заголовок		= НСтр("ru='Выберите файл обновления конфигурации'");
...
Процедура ТаблицаМетаданныхПриАктивизацииСтроки(Элемент)
	...
	Для Каждого ИмяСтраницы Из МассивСтраниц Цикл
		...
		СтрЗаголовок	= СтрШаблон("%1 (%2)", ИмяСтраницы, Формат(КолСтрок, "ЧН=0; ЧГ="));
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
			"Страница" + ИмяСтраницы, "Заголовок", СтрЗаголовок);
	КонецЦикла;
КонецПроцедуры
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Объект.ПутьКФайлуCFU		= Параметры.ПутьКФайлу;
...
Процедура ПриОткрытии(Отказ)
	...
	ЭтотОбъект.Путьzlib1		= ОбщегоНазначенияКлиентСервер.ПолучитьПолноеИмяФайла(КаталогВременныхФайлов(), "zlib1.dll");
	...
	Если Параметры.ПолучитьДанныеCFU Тогда
		Если ПустаяСтрока(Объект.ПутьКФайлуCFU) Тогда
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	НачатьУдалениеФайлов(, Объект.КаталогРаспаковки);
КонецПроцедуры
...
&НаКлиенте
Процедура РаспаковатьИВывести(Команда)
...
Процедура ОтобразитьДанныеСтрокиДерева(СтрокаДерева)
	...
	ИначеЕсли СтрокаДерева.Тип = "С" Тогда
		ТекстСправки = ПрочитатьДанныеОбъекта(СтрокаДерева.ПутьКФайлуТХТ);
...
&НаКлиентеНаСервереБезКонтекста 
Функция ПрочитатьДанныеОбъекта(знач ПутьКФайлу, знач Кодировка = Неопределено)
	Если Кодировка = Неопределено Тогда
		Кодировка = КодировкаТекста.ANSI;
	КонецЕсли;
	
	ЧтениеТекста = Новый ЧтениеТекста;
	ЧтениеТекста.Открыть(ПутьКФайлу, Кодировка);
...
Функция ПрочитатьИмяСинонимОбъекта(знач ДанныеОбъекта, знач ИдентификаторОбъекта)
	...
	Если ИндексИдентификатора <> Неопределено И ИндексИдентификатора + 1 <= КолЗаписейИнформации Тогда
		...
		Если ИндексИдентификатора + 4 <= КолЗаписейИнформации Тогда
...
Функция ПолучитьUnpackV8НаСервере()
	ОбработкаОбъект = ЭтотОбъект.РеквизитФормыВЗначение("Объект");
	Возврат ОбработкаОбъект.ПолучитьМакет("UnpackV8_Viewer");
КонецФункции

&НаСервере
Функция Получитьzlib1НаСервере()
	ОбработкаОбъект = ЭтотОбъект.РеквизитФормыВЗначение("Объект");
	Возврат ОбработкаОбъект.ПолучитьМакет("zlib1");
КонецФункции
...
&НаКлиенте
Процедура ПолучитьДанныеCFU()
	
	// открываем форму и запускаем все остальные пункты
	ФормаДлительногоОжидания = ОткрытьФорму("Обработка.ОбработкаЧтенияCFU.Форма.ФормаОжидания",,
		ЭтотОбъект,,,,,
		РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		
	// описание оповещения в результате обработки
	ОписаниеОповещения = Новый ОписаниеОповещения("ПолучитьДанныеCFUЗавершение", ЭтотОбъект);
		
	// проверяем наличие v8unpack
	ПараметрыОповещения = Новый Структура;
	ПараметрыОповещения.Вставить("ОписаниеОповещения", ОписаниеОповещения);
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ПолучитьДанныеCFU_ПроверитьUnpackV8Завершение", ЭтотОбъект, ПараметрыОповещения);
	
	ПроверкаУтилит_ПроверитьUnpackV8(ОписаниеОповещения);
	
КонецПроцедуры

&НаКлиенте
Процедура ПолучитьДанныеCFU_ПроверитьUnpackV8Завершение(ФайлСуществует, ДопПараметры) Экспорт	
	Если НЕ ФайлСуществует Тогда
		...
		ВыполнитьОбработкуОповещения(ДопПараметры.ОписаниеОповещения, РезультатВыполнения);
		Возврат;
	КонецЕсли;
	
	ПараметрыОповещения = Новый Структура;
	ПараметрыОповещения.Вставить("ОписаниеОповещения", ДопПараметры.ОписаниеОповещения);
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ПолучитьДанныеCFU_РаспаковатьЗавершение", ЭтотОбъект, ПараметрыОповещения);
	
	РаспаковатьНачало(ОписаниеОповещения);
КонецПроцедуры

&НаКлиенте
Процедура ПолучитьДанныеCFU_РаспаковатьЗавершение(РезультатРаспаковки, ДопПараметры) Экспорт	
	Если РезультатРаспаковки = Ложь Тогда
		РезультатВыполнения = ПолучитьСтруктуруРезультатОбработкиПриОткрытии(Ложь, НСтр("ru='Возникла ошибка при распаковке файла cfu.'"));
	Иначе
...
&НаКлиенте
Процедура ПолучитьДанныеCFUЗавершение(РезультатВыполнения, ДопПараметры) Экспорт	
...
&НаКлиенте 
Процедура ПроверитьНаличиеУтилит(знач ПриОткрытии = Ложь)
	ЭтотОбъект.УтилитыСуществуют = Ложь;
	
	ДопПараметры = Новый Структура;
	ДопПараметры.Вставить("ПриОткрытии", ПриОткрытии);
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ПроверитьНаличиеУтилитЗавершение", ЭтотОбъект, ДопПараметры);
	ПроверкаУтилит_ПроверитьUnpackV8(ОписаниеОповещения);
КонецПроцедуры

&НаКлиенте 
Процедура ПроверкаУтилит_ПроверитьUnpackV8(знач ОписаниеОповещения, знач Счетчик = 0)
	ДопПараметры = Новый Структура;
	ДопПараметры.Вставить("ОписаниеОповещения"	, ОписаниеОповещения);
	ДопПараметры.Вставить("Счетчик"				, Счетчик);
	
	ОповещениеПроверкаФайла = Новый ОписаниеОповещения("ПроверкаУтилит_ПроверитьUnpackV8Завершение", ЭтотОбъект, ДопПараметры);
	
	Файл = Новый Файл(ЭтотОбъект.ПутьКUnpackV8);
	Файл.НачатьПроверкуСуществования(ОповещениеПроверкаФайла);
КонецПроцедуры

&НаКлиенте
Процедура ПроверкаУтилит_ПроверитьUnpackV8Завершение(ФайлСуществует, ДопПараметры) Экспорт	
	Если ФайлСуществует = Истина Тогда
		ВыполнитьОбработкуОповещения(ДопПараметры.ОписаниеОповещения, Истина);
		//ПроверкаУтилит_Проверитьzlib1(ДопПараметры.ОписаниеОповещения);
	ИначеЕсли ДопПараметры.Счетчик = 0 Тогда 
		ДвоичныеДанныеUnpackV8 = ПолучитьUnpackV8НаСервере();
		Если ТипЗнч(ДвоичныеДанныеUnpackV8) = Тип("ДвоичныеДанные") Тогда
			ДвоичныеДанныеUnpackV8.Записать(ЭтотОбъект.ПутьКUnpackV8);
		КонецЕсли;
		
		ПроверкаУтилит_ПроверитьUnpackV8(ДопПараметры.ОписаниеОповещения, ДопПараметры.Счетчик + 1);
	Иначе
...
&НаКлиенте 
Процедура ПроверкаУтилит_Проверитьzlib1(знач ОписаниеОповещения, знач Счетчик = 0)
	ДопПараметры = Новый Структура;
	ДопПараметры.Вставить("ОписаниеОповещения"	, ОписаниеОповещения);
	ДопПараметры.Вставить("Счетчик"				, Счетчик);
	
	ОповещениеПроверкаФайла = Новый ОписаниеОповещения("ПроверкаУтилит_Проверитьzlib1Завершение", ЭтотОбъект, ДопПараметры);
	
	Файл = Новый Файл(ЭтотОбъект.Путьzlib1);
	Файл.НачатьПроверкуСуществования(ОповещениеПроверкаФайла);
КонецПроцедуры

&НаКлиенте
Процедура ПроверкаУтилит_Проверитьzlib1Завершение(ФайлСуществует, ДопПараметры) Экспорт	
	Если ФайлСуществует = Истина Тогда
		ВыполнитьОбработкуОповещения(ДопПараметры.ОписаниеОповещения, Истина);
	ИначеЕсли ДопПараметры.Счетчик = 0 Тогда 
		ДвоичныеДанныеzlib1 = Получитьzlib1НаСервере();
		Если ТипЗнч(ДвоичныеДанныеzlib1) = Тип("ДвоичныеДанные") Тогда
			ДвоичныеДанныеzlib1.Записать(ЭтотОбъект.Путьzlib1);
		КонецЕсли;
		
		ПроверкаУтилит_Проверитьzlib1(ДопПараметры.ОписаниеОповещения, ДопПараметры.Счетчик + 1);
	Иначе
		ВыполнитьОбработкуОповещения(ДопПараметры.ОписаниеОповещения, Ложь);
	КонецЕсли;
КонецПроцедуры
...
&НаКлиенте
Процедура ПроверитьНаличиеУтилитЗавершение(РезультатПроверки, ДопПараметры) Экспорт	
	ЭтотОбъект.УтилитыСуществуют = РезультатПроверки;
	УстановитьЗаголовокДекорацииНаличиеУтилит(ЭтотОбъект);
	
	МожноРаспаковать = (НЕ ПустаяСтрока(Объект.ПутьКФайлуCFU) И НЕ ПустаяСтрока(Объект.КаталогРаспаковки));
...
&НаКлиенте 
Процедура РаспаковатьИВывестиНачало() Экспорт
	
	ОписаниеОповещенияЗавершение = Новый ОписаниеОповещения("РаспаковатьИВывестиЗавершение", ЭтотОбъект);
	
	ПараметрыОповещения = Новый Структура;
	ПараметрыОповещения.Вставить("ОписаниеОповещения", ОписаниеОповещенияЗавершение);
	
	ОповещениеПостроитьДерево = Новый ОписаниеОповещения("РаспаковатьИВывести_ПостроениеДерева", ЭтотОбъект, ПараметрыОповещения);
	
	РаспаковатьНачало(ОповещениеПостроитьДерево);
	
КонецПроцедуры

&НаКлиенте 
Процедура РаспаковатьИВывести_ПостроениеДерева(РезультатРаспаковки, ДопПараметры) Экспорт
	Если РезультатРаспаковки = Истина Тогда
		Состояние(НСтр("ru='Построение дерева метаданных.'"),,, БиблиотекаКартинок.ПоказатьДанные); 
		ПостроитьДеревоМетаданныхНаСервере();
	КонецЕсли;
	
	// вызываем завершение
	ВыполнитьОбработкуОповещения(ДопПараметры.ОписаниеОповещения, РезультатРаспаковки);
КонецПроцедуры

&НаКлиенте
Процедура РаспаковатьИВывестиЗавершение(Результат, ДопПараметры) Экспорт
	Если НЕ Результат Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='При распаковке файла обновления возникли ошибки. Попробуйте еще раз.'"));
		Возврат;
	КонецЕсли;
	
	УстановитьЗаголовокИнформацияОРелизе(ЭтотОбъект);
	
	ПоказатьОповещениеПользователя(НСтр("ru='Распаковка выполнена...'"));
КонецПроцедуры
...
&НаКлиенте 
Процедура РаспаковатьНачало(знач ОписаниеОповещения)
	...
	ОповещениеОчисткаКаталога = Новый ОписаниеОповещения("Распаковать_ОчисткаКаталогаЗавершение", ЭтотОбъект, ПараметрыОповещения);
	НачатьУдалениеФайлов(ОповещениеОчисткаКаталога, Объект.КаталогРаспаковки);
	
КонецПроцедуры

&НаКлиенте 
Процедура Распаковать_ОчисткаКаталогаЗавершение(ДопПараметры) Экспорт
	
	ПутьКВременномуФайлу = ОбщегоНазначенияКлиентСервер.ПолучитьПолноеИмяФайла(КаталогВременныхФайлов(), "cfu.tmp");
	
	ПараметрыОповещения = Новый Структура;
	ПараметрыОповещения.Вставить("ОписаниеОповещения"	, ДопПараметры.ОписаниеОповещения);
	ПараметрыОповещения.Вставить("ВременныйФайл"		, ПутьКВременномуФайлу);
	
	ОповещениеПреобразованиеCFU = Новый ОписаниеОповещения("Распаковать_ПреобразованиеCFUЗавершение", ЭтотОбъект, ПараметрыОповещения);
	
	КомандаВоВремФайл = СтрШаблон("""%1"" -inflate ""%2"" ""%3""",
		ЭтотОбъект.ПутьКUnpackV8,
		Объект.ПутьКФайлуCFU,
		ПутьКВременномуФайлу);
		
	Состояние(НСтр("ru='Распаковка файла CFU...'"),, НСтр("ru='Операция может занять длительное время...'"), БиблиотекаКартинок.ПоказатьДанные); 
	...
	НачатьЗапускПриложения(ОповещениеПреобразованиеCFU, КомандаВоВремФайл,, Истина);
	
КонецПроцедуры

&НаКлиенте
Процедура Распаковать_ПреобразованиеCFUЗавершение(КодВозврата, ДопПараметры) Экспорт	
	
	ПараметрыОповещения = Новый Структура;
	ПараметрыОповещения.Вставить("ОписаниеОповещения"	, ДопПараметры.ОписаниеОповещения);
	ПараметрыОповещения.Вставить("ВременныйФайл"		, ДопПараметры.ВременныйФайл);
	
	ОповещениеРаспаковкаCFU = Новый ОписаниеОповещения("Распаковать_РаспаковкаCFUЗавершение", ЭтотОбъект, ПараметрыОповещения);
	
	КомандаРаспаковка = СтрШаблон("""%1"" -parse ""%2"" ""%3""",
		ЭтотОбъект.ПутьКUnpackV8,
		ДопПараметры.ВременныйФайл,
		Объект.КаталогРаспаковки);
		
	Состояние(НСтр("ru='Распаковка файла CFU...'"),, НСтр("ru='Операция может занять длительное время...'"), БиблиотекаКартинок.ПоказатьДанные); 
	
	//WshShell = Новый COMОбъект("WScript.Shell");
	//WshShell.Run(КомандаРаспаковка, 0, Истина);
	//
	//ВыполнитьОбработкуОповещения(ОповещениеРаспаковкаCFU, Неопределено);
	
	НачатьЗапускПриложения(ОповещениеРаспаковкаCFU, КомандаРаспаковка,, Истина);
	
КонецПроцедуры

&НаКлиенте
Процедура Распаковать_РаспаковкаCFUЗавершение(КодВозврата, ДопПараметры) Экспорт	
	НачатьУдалениеФайлов(, ДопПараметры.ВременныйФайл);
	
	ПутьКФайлуИнфо = ОбщегоНазначенияКлиентСервер.ПолучитьПолноеИмяФайла(Объект.КаталогРаспаковки, "UpdateInfo.inf");
	
	ПараметрыОповещения = Новый Структура;
	ПараметрыОповещения.Вставить("ОписаниеОповещения"	, ДопПараметры.ОписаниеОповещения);
	ПараметрыОповещения.Вставить("ФайлИнформацииCFU"	, ПутьКФайлуИнфо);
	
	ОповещениеПроверкаUpdInfo = Новый ОписаниеОповещения("Распаковать_ПроверкаUpdInfoЗавершение", ЭтотОбъект, ПараметрыОповещения);
	
	Файл = Новый Файл(ПутьКФайлуИнфо);
	Файл.НачатьПроверкуСуществования(ОповещениеПроверкаUpdInfo);
КонецПроцедуры

&НаКлиенте
Процедура Распаковать_ПроверкаUpdInfoЗавершение(ФайлСуществует, ДопПараметры) Экспорт	
	Если НЕ ФайлСуществует Тогда
		ВыполнитьОбработкуОповещения(ДопПараметры.ОписаниеОповещения, Ложь);
		Возврат;
	КонецЕсли;
	
	ИнформацияОбОбновлении = ПрочитатьДанныеОбъекта(ДопПараметры.ФайлИнформацииCFU);
	...
	Состояние(НСтр("ru='Чтение общих данных обновления'"),,, БиблиотекаКартинок.ПоказатьДанные); 
	
	ЗаполнитьРелизыОснования(МассивИнформации); 
	
	ПараметрыОповещения = Новый Структура;
	ПараметрыОповещения.Вставить("ОписаниеОповещения"	, ДопПараметры.ОписаниеОповещения);
	ПараметрыОповещения.Вставить("МассивИнформации"		, МассивИнформации);
	
	ОповещениеПоискФайлов = Новый ОписаниеОповещения("Распаковать_ПоискФайловЗавершение", ЭтотОбъект, ПараметрыОповещения);
	
	НачатьПоискФайлов(ОповещениеПоискФайлов, Объект.КаталогРаспаковки, "*", Ложь);
КонецПроцедуры

&НаКлиенте 
Процедура Распаковать_ПоискФайловЗавершение(НайденныеФайлы, ДопПараметры) Экспорт
	...
	МассивИнформации = ДопПараметры.МассивИнформации;
	...
	ИнформацияОКонфигурации		= ПрочитатьДанныеОбъекта(ФайлИнфоКонфигурация);
	...
	Для Каждого ИдентификаторИмя Из ЭтотОбъект.ИдентификаторыОбъектов Цикл
		Состояние(НСтр("ru='Обработка типа: '") + ИдентификаторИмя.Значение,,, БиблиотекаКартинок.ПоказатьДанные); 
...
Процедура ЗаполнитьТаблицуМетаданныхПоМассивуОбъектов(знач ИдентификаторИмя, знач МассивОбъектов, знач МассивИнформации, знач МассивФайлов, знач МассивРасширений, знач ИдентификаторПодсистемы = "")
	...
	Для Каждого ИдентификаторОбъекта Из МассивОбъектов Цикл
		Если МассивИнформации.Найти("""" + ИдентификаторОбъекта + """") = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		...
		ИнформацияОбОбъекте = ПрочитатьДанныеОбъекта(ФайлИнформацияОбОбъекте);
...
Процедура ДополнитьИнформациюПоТипуОбъекта(СтрокаМетаданных, знач ИнформацияОбОбъекте, знач МассивФайлов, знач МассивРасширений)
	...
	Если НЕ ПустаяСтрока(НастройкиМетаданных.ИдентификаторФормы) Тогда
		...
		Для Каждого ИдентификаторФормы Из МассивФормы Цикл
			...
			ИнформацияОФорме	= ПрочитатьДанныеОбъекта(ПутьКФайлуФормы);
	...
	Если НЕ ПустаяСтрока(НастройкиМетаданных.ИдентификаторМакеты) Тогда
		...
		Для Каждого ИдентификаторМакета Из МассивМакеты Цикл
			...
			ИнформацияОМакете	= ПрочитатьДанныеОбъекта(ПутьКФайлуМакета);
	...
	Если НЕ ПустаяСтрока(НастройкиМетаданных.ИдентификаторПерерасчеты) Тогда
		...
		Для Каждого ИдентификаторПерерасчета Из МассивПерерасчеты Цикл
			...
			ИнформацияОПерерасчете	= ПрочитатьДанныеОбъекта(ПутьКФайлуПерерасчета);
...
&НаКлиенте 
Процедура ОтобразитьСодержимоеМодуляФормы(СтрокаДанных)
	Если ПустаяСтрока(СтрокаДанных.ПутьКФайлу) Тогда
		Возврат;
	КонецЕсли;
	
	ПараметрыОповещения = Новый Структура;
	ПараметрыОповещения.Вставить("ИдентификаторСтроки"	, СтрокаДанных.ПолучитьИдентификатор());
	ПараметрыОповещения.Вставить("ПутьКФайлу"			, СтрокаДанных.ПутьКФайлу);
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ОтобразитьСодержимоеМодуляФормы_ПроверкаКаталогЗавершение", ЭтотОбъект, ПараметрыОповещения);
...
&НаКлиенте
Процедура ОтобразитьСодержимоеМодуляФормы_ПроверкаКаталогЗавершение(ФайлЭтоКаталог, ДопПараметры) Экспорт	
	СтрокаДанных = ЭтотОбъект.ДеревоМетаданных.НайтиПоИдентификатору(ДопПараметры.ИдентификаторСтроки);
	Если СтрокаДанных = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Если ФайлЭтоКаталог Тогда
		ПараметрыОповещения = Новый Структура;
		ПараметрыОповещения.Вставить("ИдентификаторСтроки"	, ДопПараметры.ИдентификаторСтроки);
		ПараметрыОповещения.Вставить("ВидДанных"			, "module");
		
		ОписаниеОповещения = Новый ОписаниеОповещения("ОтобразитьСодержимоеПакета_ПроверкаКаталогЗавершение", ЭтотОбъект, ПараметрыОповещения);
		
		ВыполнитьОбработкуОповещения(ОписаниеОповещения, ФайлЭтоКаталог);
		
		Возврат;
	КонецЕсли;
	
	ИнформацияОФорме = ПрочитатьДанныеОбъекта(СтрокаДанных.ПутьКФайлу);
...
&НаКлиенте 
Процедура ОтобразитьСодержимоеПакета(СтрокаДанных, знач ВидДанных = "text")
	Если ПустаяСтрока(СтрокаДанных.ПутьКФайлу) Тогда
		Возврат;
	КонецЕсли;
	...
	ПараметрыОповещения = Новый Структура;
	ПараметрыОповещения.Вставить("ИдентификаторСтроки"	, СтрокаДанных.ПолучитьИдентификатор());
	ПараметрыОповещения.Вставить("ВидДанных"			, ВидДанных);
...
&НаКлиенте
Процедура ОтобразитьСодержимоеПакета_ПроверкаКаталогЗавершение(ФайлЭтоКаталог, ДопПараметры) Экспорт	
	СтрокаДанных = ЭтотОбъект.ДеревоМетаданных.НайтиПоИдентификатору(ДопПараметры.ИдентификаторСтроки);
	Если СтрокаДанных = Неопределено Тогда
		Возврат;
	КонецЕсли;
	...
	ПараметрыОповещения = Новый Структура;
	ПараметрыОповещения.Вставить("ИдентификаторСтроки"	, ДопПараметры.ИдентификаторСтроки);
	ПараметрыОповещения.Вставить("КаталогРаспаковки"	, КаталогРаспаковки);
	ПараметрыОповещения.Вставить("ВидДанных"			, ДопПараметры.ВидДанных);
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ОтобразитьСодержимоеПакета_РаспаковкаЗавершение", ЭтотОбъект, ПараметрыОповещения);
	...
	Если ПустаяСтрока(СтрокаДанных.ПутьККаталогу_tmp) Тогда
		...
		НачатьЗапускПриложения(ОписаниеОповещения, КомандаРаспаковка,, Истина);
	Иначе 
...
&НаКлиенте
Процедура ОтобразитьСодержимоеПакета_РаспаковкаЗавершение(КодВозврата, ДопПараметры) Экспорт	
	СтрокаДерева = ЭтотОбъект.ДеревоМетаданных.НайтиПоИдентификатору(ДопПараметры.ИдентификаторСтроки);
	Если СтрокаДерева = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	СтрокаДерева.ПутьККаталогу_tmp = ДопПараметры.КаталогРаспаковки;
	
	ПараметрыОповещения = Новый Структура;
	ПараметрыОповещения.Вставить("ИдентификаторСтроки"	, ДопПараметры.ИдентификаторСтроки);
	ПараметрыОповещения.Вставить("КаталогРаспаковки"	, ДопПараметры.КаталогРаспаковки);
	ПараметрыОповещения.Вставить("ВидДанных"			, ДопПараметры.ВидДанных);
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ОтобразитьСодержимоеПакета_ПоискФайловЗавершение", ЭтотОбъект, ДопПараметры);
	
	НачатьПоискФайлов(ОписаниеОповещения, СтрокаДерева.ПутьККаталогу_tmp, "*");
КонецПроцедуры

&НаКлиенте
Процедура ОтобразитьСодержимоеПакета_ПоискФайловЗавершение(НайденныеФайлы, ДопПараметры) Экспорт	
	СтрокаДерева = ЭтотОбъект.ДеревоМетаданных.НайтиПоИдентификатору(ДопПараметры.ИдентификаторСтроки);
	Если СтрокаДерева = Неопределено Тогда
		Возврат;
	КонецЕсли;
	...
	Если НЕ ЕстьНайденныеФайлы Тогда
		Если СтрокаДерева.Тип = "М" Тогда
			ОтобразитьСодержимоеМакета(СтрокаДерева);
		ИначеЕсли НЕ СтрокаДерева.Тип = "МФ" Тогда
			ОтобразитьДанныеСтрокиДерева(СтрокаДерева);
		КонецЕсли;
	...
	Для Каждого Файл Из НайденныеФайлы Цикл
		Если Файл.ИмяБезРасширения = ДопПараметры.ВидДанных Тогда
			СтрокаДерева.ПутьКФайлуТХТ = Файл.ПолноеИмя;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	...
	Если ТекущиеДанные.ПолучитьИдентификатор() = ДопПараметры.ИдентификаторСтроки Тогда
		ОтобразитьДанныеСтрокиДерева(ТекущиеДанные);
	КонецЕсли;
КонецПроцедуры
...
Процедура ОтобразитьСодержимоеСправки(СтрокаДанных)
	...
	ИнформацияСправки = ПрочитатьДанныеОбъекта(СтрокаДанных.ПутьКФайлу);
...
Процедура ОтобразитьСодержимоеКартинки(СтрокаДанных)
	...
	ИнформацияКартинки = ПрочитатьДанныеОбъекта(СтрокаДанных.ПутьКФайлу);
...
Процедура ОтобразитьСодержимоеМакета(СтрокаДанных)
	ТекстМакета = ПрочитатьДанныеОбъекта(СтрокаДанных.ПутьКФайлу, КодировкаТекста.UTF8);
